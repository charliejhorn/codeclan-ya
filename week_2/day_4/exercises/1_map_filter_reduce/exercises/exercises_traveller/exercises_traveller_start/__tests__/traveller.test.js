...
describe('Traveller', () => {
    ...
    xit('should have a collection of journeys', () => {
        const actual = traveller.journeys;
        expect(actual).toEqual(journeys);
    });

    xit('should be able to get the journeys start locations', () => {
        const expected = [journey1.startLocation, journey2.startLocation, journey3.startLocation, journey4.startLocation, journey5.startLocation];
        expect(traveller.getJourneyStartLocations()).toEqual(expected);
    });

    xit('should be able to get the journeys end locations', () => {
        const expected = [journey1.endLocation, journey2.endLocation, journey3.endLocation, journey4.endLocation, journey5.endLocation];
        expect(traveller.getJourneyEndLocations()).toEqual(expected);
    });

    xit('should be able to get journeys by transport', () => {
        const expected = [journey1, journey2];
        expect(traveller.getJourneysByTransport('train')).toEqual(expected);
    });

    xit('should be able to get journeys over a certain distance', () => {
        const expected = [journey3, journey4];
        expect(traveller.getJourneysByMinDistance(1000)).toEqual(expected);
    });

    xit('should be able to calculate total distance travelled', () => {
        expect(traveller.calculateTotalDistanceTravelled()).toBe(11710);
    });

    xit('should be able to get a unique list of the modes of transport', () => {
